 ...
 * Download ZIP com PNG + ficheiro de georeferenciação (.pgw) de um raster de satélite.
...
/**
 * Gera o conteúdo de um ficheiro world (.pgw) a partir dos limites geográficos
 * e das dimensões do PNG.
 *   Linha 1: tamanho do pixel em longitude (graus/px)
 *   Linha 2: rotação eixo Y (0)
 *   Linha 3: rotação eixo X (0)
 *   Linha 4: tamanho do pixel em latitude (graus/px, negativo)
 *   Linha 5: longitude do centro do pixel superior-esquerdo
 *   Linha 6: latitude do centro do pixel superior-esquerdo
 */
...
try {
    $pdo = getDBConnection();
    $st  = $pdo->prepare("
        SELECT name, raster_type, png_data, min_lat, max_lat, min_lng, max_lng
        FROM raster_results
    ...
    $zipBuf = new SplTempFileObject(-1);
    $zipPath = tempnam(sys_get_temp_dir(), 'soilqi_raster_');
    $zip = new ZipArchive();
