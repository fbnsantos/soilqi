<?php
/**
 * api/interpolation_download_png.php
 * Download ZIP com PNG + ficheiro de georeferenciação (.pgw) de uma interpolação de campo.
 * GET: ?id=<interpolation_id>  (requer sessão autenticada)
 */
...
try {
    $pdo = getDBConnection();
    $st  = $pdo->prepare("
        SELECT name, png_data, min_lat, max_lat, min_lng, max_lng
    ...
    $safe = substr($safe, 0, 60) ?: 'interpolacao';
    ...
    $zipPath = tempnam(sys_get_temp_dir(), 'soilqi_interp_');
    $zip = new ZipArchive();
